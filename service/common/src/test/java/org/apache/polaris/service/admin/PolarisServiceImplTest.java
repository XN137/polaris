...
public class PolarisServiceImplTest {
  ...
  private PolarisRealmConfiguration realmConfig;
  ...
  void setUp() {
    ...
    realmConfig = Mockito.mock(PolarisRealmConfiguration.class);

    when(callContext.getRealmConfiguration()).thenReturn(realmConfig);
    when(realmConfig.getConfiguration(FeatureConfiguration.SUPPORTED_CATALOG_CONNECTION_TYPES))
        .thenReturn(List.of("ICEBERG_REST"));
    when(realmConfig.getConfiguration(
