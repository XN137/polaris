...
public class IcebergPropertiesValidation {
  ...
  public static String determineFileIOClassName(
    ...
    var realmConfig = callContext.getPolarisCallContext().getRealmConfiguration();
    var ioImpl = properties.get(CatalogProperties.FILE_IO_IMPL);

    if (ioImpl != null) {
      if (!realmConfig.getConfiguration(ALLOW_SPECIFYING_FILE_IO_IMPL)) {
    ...
    if (ioImpl != null) {
      ...
      if (!storageType.safe() && !realmConfig.getConfiguration(ALLOW_INSECURE_STORAGE_TYPES)) {
        throw new ValidationException(
