...
public abstract class JWTBroker implements TokenBroker {
  ...
  JWTBroker(MetaStoreSession metaStoreSession, int maxTokenGenerationInSeconds) {
  ...
  public TokenResponse generateFromToken(
    ...
    Optional<PrincipalEntity> principalLookup =
        metaStoreSession.findPrincipalById(decodedToken.getPrincipalId());
  ...
  private Optional<PrincipalEntity> findPrincipalEntity(String clientId, String clientSecret) {
    ...
    PrincipalSecretsResult principalSecrets = metaStoreSession.loadPrincipalSecrets(clientId);
    ...
    return metaStoreSession.findPrincipalById(
