...
public class DefaultAuthenticator implements Authenticator {
  ...
  protected PrincipalEntity resolvePrincipalEntity(PolarisCredential credentials) {
    ...
    try {
      ...
      if (credentials.getPrincipalId() != null && credentials.getPrincipalId() > 0) {
        principal = metaStoreSession.findPrincipalById(credentials.getPrincipalId()).orElse(null);
      } else if (credentials.getPrincipalName() != null) {
        principal =
            metaStoreSession.findPrincipalByName(credentials.getPrincipalName()).orElse(null);
      }
    } catch (Exception e) {
      LOGGER
          .atError()
  ...
  protected LoadGrantsResult loadPrincipalGrants(PrincipalEntity principal) {
    LoadGrantsResult principalGrantResults = metaStoreSession.loadGrantsToGrantee(principal);
    diagnostics.check(
