...
public class PolarisIntegrationTestFixture {
  ...
  private PolarisPrincipalSecrets fetchAdminSecrets() {
    ...
    MetaStore metaStore = helper.metaStoreFactory.create(realmContext);
    PrincipalEntity principal = metaStore.findRootPrincipal().orElseThrow();
    return metaStore.loadPrincipalSecrets(principal.getClientId()).getPrincipalSecrets();
  }
