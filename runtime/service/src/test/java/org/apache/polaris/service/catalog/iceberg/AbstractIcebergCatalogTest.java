...
public abstract class AbstractIcebergCatalogTest extends CatalogTests<IcebergCatalog> {
  ...
  public void before(TestInfo testInfo) {
    ...
    resolverFactory =
        (principal, referenceCatalogName) ->
            new Resolver(diagServices, metaStore, principal, entityCache, referenceCatalogName);
  ...
  protected IcebergCatalog newIcebergCatalog(String catalogName) {
    return newIcebergCatalog(catalogName, metaStore);
  }

  protected IcebergCatalog newIcebergCatalog(String catalogName, MetaStore metaStore) {
    return newIcebergCatalog(catalogName, metaStore, fileIOFactory);
  }

  protected IcebergCatalog newIcebergCatalog(
      String catalogName, MetaStore metaStore, FileIOFactory fileIOFactory) {
  ...
  public void testValidateNotificationFailToCreateFileIO() {
    ...
    IcebergCatalog catalog = newIcebergCatalog(catalog().name(), metaStore, fileIOFactory);
    catalog.initialize(
        CATALOG_NAME,
        ImmutableMap.of(
  ...
  public void testUpdateNotificationWhenTableAndNamespacesDontExistNamespaceRaceCondition() {
    ...
    MetaStore spyMetaStore = spy(metaStore);
  ...
  public void testDropTableWithPurge() {
    ...
    List<PolarisBaseEntity> tasks =
        metaStore.loadTasks("testExecutor", PageToken.fromLimit(1)).getEntities();
    ...
    adminService.createCatalog(
        new CreateCatalogRequest(
            new CatalogEntity.Builder()
                ...
                .build()
                .asCatalog(serviceIdentityProvider)));
    IcebergCatalog noPurgeCatalog = newIcebergCatalog(noPurgeCatalogName, metaStore, fileIOFactory);
  ...
  public void testFileIOWrapper() {
    ...
    IcebergCatalog catalog = newIcebergCatalog(CATALOG_NAME, metaStore, measured);
    catalog.initialize(
        CATALOG_NAME,
        ImmutableMap.of(
            CatalogProperties.FILE_IO_IMPL, "org.apache.iceberg.inmemory.InMemoryFileIO"));
    ...
    TaskEntity taskEntity =
        TaskEntity.of(
            metaStore.loadTasks("testExecutor", PageToken.fromLimit(1)).getEntities().getFirst());
    ...
    TaskContext taskContext = new TaskContext(realmContext, realmConfig, metaStore);
