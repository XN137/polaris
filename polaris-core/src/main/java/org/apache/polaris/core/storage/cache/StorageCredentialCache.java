...
public class StorageCredentialCache {
  ...
  public StorageCredentialCache(PolarisRealmConfiguration realmConfig) {
  ...
  private long maxCacheDurationMs() {
    var cacheDurationSeconds =
        realmConfig.getConfiguration(
            FeatureConfiguration.STORAGE_CREDENTIAL_CACHE_DURATION_SECONDS);
    var credentialDurationSeconds =
        realmConfig.getConfiguration(FeatureConfiguration.STORAGE_CREDENTIAL_DURATION_SECONDS);
