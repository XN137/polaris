...
public class InMemoryEntityCache implements EntityCache {
   ...
   * specified version.
   *
   * @param metaStoreSession the metastore session to use for persistence access
  ...
  public @Nullable ResolvedPolarisEntity getAndRefreshIfNeeded(
    ...
    if (existingCacheEntry == null
      ...
      if (existingCacheEntry == null) {
        // try to load it
        refreshedCacheEntry =
            metaStoreSession.loadResolvedEntityById(entityCatalogId, entityId, entityType);
        ...
        // refresh it
        refreshedCacheEntry =
            metaStoreSession.refreshResolvedEntity(
   ...
   *
   * @param metaStoreSession the metastore session to use for loading
  ...
  public @Nullable EntityCacheLookupResult getOrLoadEntityByName(
      @Nonnull MetaStoreSession metaStoreSession, @Nonnull EntityCacheByNameKey entityNameKey) {
    ...
    if (entry == null) {
      ...
      // load it
      ResolvedEntityResult result =
          metaStoreSession.loadResolvedEntityByName(
  ...
  public List<EntityCacheLookupResult> getOrLoadResolvedEntities(
    ...
    for (int i = 0; i < MAX_CACHE_REFRESH_ATTEMPTS; i++) {
      Function<List<PolarisEntityId>, ResolvedEntitiesResult> loaderFunc =
          idsToLoad -> metaStoreSession.loadResolvedEntities(entityType, idsToLoad);
      if (isCacheStateValid(metaStoreSession, resolvedEntities, entityIds, loaderFunc)) {
        stateResolved = true;
        break;
      }
    }
  ...
  private boolean isCacheStateValid(
    ...
    ChangeTrackingResult changeTrackingResult =
        metaStoreSession.loadEntitiesChangeTracking(entityIds);
