...
import org.apache.polaris.core.persistence.session.ForwardingMetaStoreSession;
...
public class InMemoryEntityCacheTest {
  ...
  public InMemoryEntityCacheTest() {
    ...
    PolarisCallContext callCtx = new PolarisCallContext(realmContext, metaStore);
    realmConfig = callCtx.getRealmConfig();
    PolarisMetaStoreManager metaStoreManager =
        new TransactionalMetaStoreManagerImpl(Clock.systemUTC(), diagServices);
    ...
    metaStoreSession = new ForwardingMetaStoreSession(metaStoreManager, callCtx);
  }
  ...
  void testGetOrLoadEntityByName() {
    ...
    EntityCacheLookupResult lookup =
        cache.getOrLoadEntityByName(
            metaStoreSession, new EntityCacheByNameKey(PolarisEntityType.CATALOG, "test"));
    ...
    lookup =
        cache.getOrLoadEntityById(
            metaStoreSession, catalog.getCatalogId(), catalog.getId(), catalog.getType());
    assertThat(lookup).isNotNull();
    assertThat(lookup.isCacheHit()).isTrue();
    assertThat(lookup.getCacheEntry()).isNotNull();
    ...
    lookup =
        cache.getOrLoadEntityById(metaStoreSession, N1.getCatalogId(), N1.getId(), N1.getType());
    assertThat(lookup).isNotNull();
    assertThat(lookup.isCacheHit()).isFalse();
    ...
    lookup = cache.getOrLoadEntityById(metaStoreSession, N1.getCatalogId(), 10000, N1.getType());
    ...
    lookup = cache.getOrLoadEntityByName(metaStoreSession, N2_name);
    assertThat(lookup).isNotNull();
    ...
    lookup = cache.getOrLoadEntityByName(metaStoreSession, R1_name);
    assertThat(lookup).isNotNull();
    ...
    lookup = cache.getOrLoadEntityByName(metaStoreSession, PR1_name);
    assertThat(lookup).isNotNull();
  ...
  void testRefresh() {
    ...
    cacheEntry =
        cache.getAndRefreshIfNeeded(
            metaStoreSession, T6v1, T6v1.getEntityVersion(), T6v1.getGrantRecordsVersion());
    assertThat(cacheEntry).isNotNull();
    assertThat(cacheEntry.getEntity()).isNotNull();
    assertThat(cacheEntry.getGrantRecordsAsSecurable()).isNotNull();
    ...
    cacheEntry =
        cache.getAndRefreshIfNeeded(
            metaStoreSession, T6v2, T6v2.getEntityVersion(), T6v2.getGrantRecordsVersion());
    assertThat(cacheEntry).isNotNull();
    assertThat(cacheEntry.getEntity()).isNotNull();
    assertThat(cacheEntry.getGrantRecordsAsSecurable()).isNotNull();
    table = cacheEntry.getEntity();
    ...
    cacheEntry =
        cache.getAndRefreshIfNeeded(
            metaStoreSession, N2, N2.getEntityVersion(), N2.getGrantRecordsVersion());
    ...
    cacheEntry =
        cache.getAndRefreshIfNeeded(
            metaStoreSession, N2, N2v2.getEntityVersion(), N2v2.getGrantRecordsVersion());
    assertThat(cacheEntry).isNotNull();
    assertThat(cacheEntry.getEntity()).isNotNull();
    assertThat(cacheEntry.getGrantRecordsAsSecurable()).isNotNull();
  ...
  void testRenameAndCacheDestinationBeforeLoadingSource() {
    ...
    lookup = cache.getOrLoadEntityByName(metaStoreSession, T4_name);
    assertThat(lookup).isNotNull();
    ...
    lookup = cache.getOrLoadEntityByName(metaStoreSession, T4_renamed);
    assertThat(lookup).isNotNull();
    ...
    EntityCacheLookupResult lookupResult =
        cache.getOrLoadEntityById(metaStoreSession, T4.getCatalogId(), T4.getId(), T4.getType());
    ...
    assertThat(cache.getOrLoadEntityByName(metaStoreSession, T4_name)).isNull();
  }
  ...
  public void testBatchLoadByEntityIds() {
    ...
    List<EntityCacheLookupResult> results =
        cache.getOrLoadResolvedEntities(metaStoreSession, PolarisEntityType.NAMESPACE, entityIds);
    ...
    List<EntityCacheLookupResult> nonExistentResults =
        cache.getOrLoadResolvedEntities(
            metaStoreSession, PolarisEntityType.NAMESPACE, nonExistentIds);
    ...
    List<EntityCacheLookupResult> tableResults =
        cache.getOrLoadResolvedEntities(metaStoreSession, PolarisEntityType.TABLE_LIKE, tableIds);
  ...
  public void testBatchLoadWithStaleVersions() {
    ...
    cache.getOrLoadEntityById(metaStoreSession, T6v1.getCatalogId(), T6v1.getId(), T6v1.getType());
    ...
    results =
        cache.getOrLoadResolvedEntities(metaStoreSession, PolarisEntityType.TABLE_LIKE, entityIds);
  ...
  public void testBatchLoadVersionRetryLogic() {
    MetaStoreSession metaStoreSession = Mockito.spy(this.metaStoreSession);
    // get a new cache
    InMemoryEntityCache cache = new InMemoryEntityCache(diagServices, realmConfig);
    ...
    Mockito.doReturn(
        ...
        .when(metaStoreSession)
        .loadEntitiesChangeTracking(Mockito.any());
    Mockito.doCallRealMethod().when(metaStoreSession).loadEntitiesChangeTracking(Mockito.any());
  ...
  public void testBatchLoadVersionRetryFailsAfterMaxAttempts() {
    ...
    Assertions.assertThatThrownBy(
            () ->
                cache.getOrLoadResolvedEntities(
                    metaStoreSession, PolarisEntityType.NAMESPACE, entityIds))
  ...
  public void testConcurrentClientLoadingBehavior() throws Exception {
    ...
    // Mock the metastore session to control the timing of method calls
    MetaStoreSession mockedMetaStoreSession = Mockito.spy(this.metaStoreSession);
              ...
              () -> {
                try {
                  ...
                  return cache.getOrLoadResolvedEntities(
                      mockedMetaStoreSession, PolarisEntityType.NAMESPACE, entityIds);
                } catch (Exception e) {
