...
public class PolarisTreeMapMetaStoreManagerTest extends BasePolarisMetaStoreManagerTest {
  ...
  public PolarisTestMetaStoreManager createPolarisTestMetaStoreManager() {
    ...
    TreeMapTransactionalPersistenceImpl treeMapTransactionalPersistence =
        new TreeMapTransactionalPersistenceImpl(store, Mockito.mock(), RANDOM_SECRETS);
    TransactionalMetaStoreManagerImpl metaStoreManager = new TransactionalMetaStoreManagerImpl();
    PolarisCallContext callCtx =
        new PolarisCallContext(
            () -> "testRealm",
            () -> treeMapTransactionalPersistence,
            () -> metaStoreManager,
            diagServices,
            new PolarisConfigurationStore() {},
            timeSource.withZone(ZoneId.systemDefault()));

    return new PolarisTestMetaStoreManager(callCtx);
  }
}
